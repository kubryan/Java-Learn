...
function vitePluginLocalKnowledgeManager(): Plugin {
  ...
  const backup = (target: string) => { if (!fs.existsSync(target)) return; fs.mkdirSync(backupRoot, { recursive: true }); fs.copyFileSync(target, path.join(backupRoot, `${Date.now()}-${path.basename(target)}`)); };
