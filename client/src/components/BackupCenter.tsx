...
import { createLocalBackup, downloadBackup, exportMarkdownFile, exportMarkdownWorkspace, exportWorkspaceArchive, importMarkdownFile, restoreLocalBackup, rescanMarkdownWorkspace, summarizeBackup, validateLocalBackup, type BackupSummary, type JavaBaseBackup } from "@/lib/local-backup";
...
export function BackupCenter({ note, open, onOpenChange }: { note: Note; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  async function exportFullWorkspace() {
    setLoading(true); setNotice("");
    try { await exportWorkspaceArchive(); setNotice("已匯出完整本地 Workspace ZIP，包含 Markdown、Workspace Assets 與 File Relations metadata；所有內容仍只在本地處理。"); }
    catch (error) { setNotice(error instanceof Error ? error.message : "完整 Workspace 匯出未完成。 "); }
    finally { setLoading(false); }
  }
      ...
      <DialogContent className="max-h-[calc(100vh-2rem)] max-w-5xl overflow-y-auto border-slate-950/20 bg-[#f8f4e9] p-0 text-slate-950 shadow-2xl">
        ...
        <div className="space-y-5 px-6 py-5 sm:px-8">
          ...
          <section className="backup-index-card p-4">
            <div className="flex items-start gap-3"><RefreshCw className="mt-0.5 h-5 w-5 text-teal-800" /><div><p className="section-label text-teal-800">MARKDOWN WORKSPACE TOOLS</p><h3 className="mt-1 font-serif text-lg font-bold">匯入、匯出與同步</h3><p className="mt-1 text-sm leading-6 text-slate-600">Markdown 是真正的 Knowledge Source。你可以在 VS Code 修改檔案後按「重新掃描 Markdown」；其他本地檔案由 Workspace Assets 管理，只有 `.txt` 會進入可丟棄的搜尋索引。</p></div></div>
            ...
            <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-5">
              ...
              <button type="button" onClick={() => void exportWholeWorkspace()} disabled={loading} className="completion-stamp justify-center"><Download className="h-4 w-4" />匯出 Markdown 知識庫</button>
              <button type="button" onClick={() => void exportFullWorkspace()} disabled={loading} className="completion-stamp justify-center"><HardDriveDownload className="h-4 w-4" />匯出完整 Workspace</button>
          ...
          <section className="rounded-md border border-slate-950/10 bg-slate-950/[0.035] p-4"><p className="section-label">SAFETY BOUNDARY</p><p className="mt-1 text-sm leading-6 text-slate-700">備份 JSON 是「瀏覽器本機狀態」的可攜副本；它不會複製整個 workspace。Markdown、Assets 與 File Relations 請使用「匯出完整 Workspace」或 Git／檔案備份；任何 workspace 檔案都不會上傳雲端。</p></section>
        </div>
      </DialogContent>
    </Dialog>
