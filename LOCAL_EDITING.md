...
本文件以**專案根目錄**表示路徑；請把 `<project-root>` 替換成你實際複製或 clone 的資料夾。**`client/src/content/` 內的 Markdown 是知識庫的唯一原始來源**；網站、全文搜尋、知識樹與瀏覽器 IndexedDB 都由這些實體檔案建立索引。

## 先安裝一次開發環境

請先安裝 [Node.js LTS](https://nodejs.org/)。不需要全域安裝 pnpm，也不需要 `corepack enable`；進入專案根目錄後，直接使用 Corepack 執行：
...
Windows 使用者也可以雙擊專案根目錄的 `start-local.bat`，它會自動找到 Node.js、安裝缺少的依賴並啟動伺服器。macOS／Linux 可使用上面的 shell 命令；若要長期使用，也可以自行建立對應的 `.command` 或 shell script。
...
如果專案尚未初始化為 Git 儲存庫，請在終端機進入專案根目錄後，確認你要使用自己的 GitHub 帳號與遠端網址，再執行下列設定一次。Windows PowerShell、macOS Terminal 與 Linux shell 都可使用相同命令：
...
完成設定、重新執行 `corepack pnpm dev` 後按「重新檢查」，就能看到 Markdown 變更。`Commit` 只建立本機版本；`Push` 會在另一個確認視窗中才推送目前分支到 `origin`。若 GitHub 要求登入，請在 Windows 的 Git Credential Manager 或 GitHub CLI 完成登入後再 Push。若這個遠端已經有不同歷史，請先以命令列檢查並整合歷史，**不要**在工作台中強制推送。
...
在文章工具列選擇「**編輯實體 Markdown**」後，停止輸入約 **1.2 秒**，工作台就會自動把內容保存到對應的 `.md`。預覽欄會清楚顯示「正在編輯」、「正在自動保存」或「已保存」及時間；`Ctrl+S`／`Cmd+S` 仍可用於立即保存。
